...
int main(int argc, char *argv[]) {
  ...
  int thirdVal = atoi(argv[3]);

  //Growth amount
  if(firstVal >= 5 && firstVal <= 100)
    environment.growthAmount = firstVal;
  else{
    printf("Growth amount must be between 5 and 100");
    return 0;
  }

  //Maximum nodes
  if(secondVal >= 5 && secondVal <= 5000)
    environment.maximumNodes = secondVal;
  else{
    printf("Maximum nodes must be between 5 and 5000");
    return 0;
  }

  //Environment number
  if(thirdVal >= 1 && thirdVal <= 5)
    environmentNumber = thirdVal;
  else{
    printf("Environment number must be between 1 and 5");
    return 0;
  }
